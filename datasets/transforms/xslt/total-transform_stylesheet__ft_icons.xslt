<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template  name="luogo">
	...
	</xsl:choose>-->r_blue</xsl:when><xsl:when test="tipi-specifici = 'casa ferie'">f_blue</xsl:when><xsl:when test="contains(tipi-specifici,'discoteca')">pink_diamond</xsl:when><xsl:when test="contains(tipi-specifici,'discopub')">purple_diamond</xsl:when><xsl:when test="contains(tipi-specifici,'wine bar')">bars</xsl:when><xsl:when test="contains(tipi-specifici,'cocktail bar')">bars</xsl:when><xsl:when test="contains(tipi-specifici,'bar')">coffee</xsl:when><xsl:when test="tipi-specifici = 'ristorante'">dining</xsl:when><xsl:when test="tipi-specifici = 'pizzeria'">rec_dining</xsl:when><xsl:when test="contains(tipi-specifici,'ristorante') and contains(tipi-specifici,'pizzeria')">rec_dining</xsl:when><xsl:when test="contains(tipi-specifici,'pub')">snack_bar</xsl:when><xsl:when test="contains(tipi-specifici,'trattoria')">dining</xsl:when><xsl:when test="contains(tipi-specifici,'sushi')">yen</xsl:when><xsl:when test="contains(tipi-specifici,'focacceria') or contains(tipi-specifici,'street food')">snack_bar</xsl:when><xsl:otherwise>red_blank</xsl:otherwise></xsl:choose></icon>
      </luogo>
   </xsl:for-each>
  </xsl:template>
